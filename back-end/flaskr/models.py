...
class UserEmail(db.Model):
    ...
    def get_user_received_emails(cls, userId):
        statement = db.select(cls).join(Email, cls.emailId == Email.emailId).where(cls.recipentId == userId)
        relationship_info = db.session.execute(statement).scalars()
        user_email_schema = UserEmailSchema()
        return user_email_schema.get_users_emails(relationship_info)
